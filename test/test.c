...
// static BOOL CALLBACK EnumMonCb(HMONITOR hMon, HDC hdc, LPRECT rc, LPARAM lp) {
//     (void)hdc;
//     (void)rc;
//     (void)lp;
...
//     wprintf(L"%ls%s\n", mi.szDevice, (mi.dwFlags & MONITORINFOF_PRIMARY) ? L" (PRIMARY)" : L"");

//     /* Per-monitor DC */
//     HDC hdcMon = CreateDCW(L"DISPLAY", mi.szDevice, NULL, NULL);
//     if (hdcMon) {
//         int w = GetDeviceCaps(hdcMon, HORZRES);
//         int h = GetDeviceCaps(hdcMon, VERTRES);
//         int pw = GetDeviceCaps(hdcMon, DESKTOPHORZRES);
//         int ph = GetDeviceCaps(hdcMon, DESKTOPVERTRES);

//         wprintf(L"  HORZRES=%d VERTRES=%d  DESKTOPHORZRES=%d DESKTOPVERTRES=%d\n", w, h, pw, ph);

//         DeleteDC(hdcMon);
//     } else {
//         wprintf(L"  CreateDCW failed (err=%lu)\n", GetLastError());
//     }
...
// int main(void) {
//     if (!EnumDisplayMonitors(NULL, NULL, EnumMonCb, 0)) {
//         printf("EnumDisplayMonitors failed (err=%lu)\n", GetLastError());
//         return 1;
//     }
//     return 0;
// }